...
- Search page: new **Start Date** and **End Date** fields under the existing Date field, each with their own calendar-icon picker, for searching a specific date range instead of a single day or a substring match. A flight matches if it was active at any point during the range — its First DateTime on/before End Date and its Last DateTime on/after Start Date — so a flight that started before the range or ran past it still shows up if it was in the air at some point inside it. Filling in either field switches the query into range mode for that search and the plain Date field above is ignored, rather than trying to combine a substring match with a real range; both Start and End are required together, and Start must be on or before End, with an on-page message if either condition isn't met. A range of up to 7 days can be searched on its own — handy combined with just "Show only flagged" to check the last few days for anything flagged — but a longer range needs an ICAO24, Registration, or Callsign value too, same reasoning as the bare year/month protection above.
